...
//            // 判断是否碰到边框
//            isTouchTopBorder = member.X + memberData.SpaceSet >= MapPlane.transform.localPosition.x + MapWidth;
//            isTouchBottomBorder = member.X <= MapPlane.transform.localPosition.x;
//            isTouchRightBorder = member.Y + memberData.SpaceSet >= MapPlane.transform.localPosition.z + MapHeight;
...
//        // 根据对象的搜寻外径获取对向列表
//        var itemRect = new Rectangle(item.X - memberData.AttackRange * 0.5f, item.Y - memberData.AttackRange * 0.5f, memberData.AttackRange,
//                memberData.AttackRange);
...
//    private void DrawQuadTreeLine<T>(QuadTree<T> quadTree) where T : IBaseMember, IGraphical<Rectangle>
//    {
//        // 绘制四叉树边框
